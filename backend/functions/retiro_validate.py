...
from globals.retiro_vars import (
    ERROR_MESSAGE_7AMPM,
    ERROR_MESSAGE_MONTO,
    RETIRO_PERMITIDO_DESDE,
    RETIRO_PERMITIDO_HASTA,
    USUARIO_MINUTOS_BLOQUEO,
)
    ...
    if not (7 <= hora.hour < 19):
        if hora.hour == 19:
            if hora.minute > 0 or hora.second > 0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail=ERROR_MESSAGE_7AMPM
                )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=ERROR_MESSAGE_7AMPM
...
def raise_validar_monto_retiro(monto: float):
    if not (10_000 <= monto <= 1_000_000):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=ERROR_MESSAGE_MONTO
        )


def manejar_intento_fallido(
    db: Session, id_usuario: int, error_detail: str = "Error en la operación"
):
    """
    Maneja la lógica de intentos fallidos y bloqueo de usuario
    """
    user_to_update = (
        db.query(UsuarioModel).filter(UsuarioModel.id == id_usuario).first()
    )
    ...
    if intentos >= 3:
        ...
        return HTTPException(status_code=status.HTTP_423_LOCKED, detail=error_response)
    ...
    return HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=error_response)
